      ...
      <div class="voice-info">
        ...
        <div class="voice-meta">
          <span
            ...
            :key="i"
            class="tag tag-primary tag-editable"
            @click.stop="startEditTag(i)"
          >
            {{ tag }}
        ...
        <div v-if="editing && editingIdx >= 0" class="tag-edit-row">
          <input v-model="editTagValue" placeholder="输入标签，用逗号分隔" @keyup.enter="saveTag" class="tag-input" />
...
import { User, Baby, Star, Heart, Play, X, Trash2, Settings2 } from 'lucide-vue-next'
...
const emit = defineEmits(["toggleFavorite", "preview", "select", "deleteVoice", "updateTags"])
...
function startEditTag(i) {
  ...
  editingIdx.value = i
  editTagValue.value = editableTags.value[i] || ""
}
...
function addTag() {
  const tags = [...editableTags.value]
  tags.push("")
  editingIdx.value = tags.length - 1
  editTagValue.value = ""
  emit("updateTags", tags)
}
</script>
...
.voice-avatar.male { background: #dbeafe; color: #3b82f6; }
...
.fav-btn { background: none; padding: 4px; border-radius: 50%; flex-shrink: 0; transition: transform var(--transition); color: var(--text-muted); }
...
.tag-add { border: 1px dashed #6366f1; color: #6366f1; background: #fff; cursor: pointer; }
.tag-add:hover { background: #eef2ff; }
